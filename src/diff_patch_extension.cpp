	...
	BinaryExecutor::Execute<string_t, string_t, string_t>(
		...
		while ((elem = yyjson_arr_iter_next(&it))) {
			...
			if (tag[0] == '=' && tag[1] == '\0') {
				...
				if (idx > old_str.size()) idx = old_str.size();
...
static void LoadInternal(DatabaseInstance &instance) {
	...
	auto diff_patch_scalar_function = ScalarFunction("diff_patch", {LogicalType::VARCHAR, LogicalType::VARCHAR}, LogicalType::VARCHAR, DiffPatchScalarFun);
